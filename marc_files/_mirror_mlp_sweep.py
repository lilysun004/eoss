"""One-off helper: produce PNG + PDF histograms for every MLP_sweep cell.

For each unique (optimizer, lr, batch, params) cell under $RESULTS/MLP_sweep/,
this picks the latest run folder (by timestamp prefix), then calls
plot_histograms.py twice — once for PNG (under results_histograms/MLP_sweep/<opt>/),
once for PDF (under results_histograms_pdf/MLP_sweep/<opt>/).
...
RESULTS_ROOT = Path("/n/holylabs/LABS/kdbrantley_lab/Lab/mwalden/results/MLP_sweep")
PNG_ROOT = Path("/n/home06/mwalden/eoss/marc_files/results_histograms/MLP_sweep")
PDF_ROOT = Path("/n/home06/mwalden/eoss/marc_files/results_histograms_pdf/MLP_sweep")
...
def main() -> None:
    ...
    for (opt, lr, batch, rest), runs in sorted(cells.items()):
        # Latest by timestamp prefix (folder name sorts chronologically)
