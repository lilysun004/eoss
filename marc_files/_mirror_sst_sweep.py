"""One-off helper: produce PNG + PDF histograms for every SST_opt_batch_sweep cell.

For each unique (optimizer, lr, batch, params) cell under $RESULTS/SST_opt_batch_sweep/,
this picks the latest run folder (by timestamp prefix), then calls
plot_histograms.py twice — once for PNG (under results_histograms/SST_opt_batch_sweep/<opt>/),
once for PDF (under results_histograms_pdf/SST_opt_batch_sweep/<opt>/).
...
RESULTS_ROOT = Path("/n/holylabs/LABS/kdbrantley_lab/Lab/mwalden/results/SST_opt_batch_sweep")
PNG_ROOT = Path("/n/home06/mwalden/eoss/marc_files/results_histograms/SST_opt_batch_sweep")
PDF_ROOT = Path("/n/home06/mwalden/eoss/marc_files/results_histograms_pdf/SST_opt_batch_sweep")
