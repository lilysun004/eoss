"""
GPU job scheduler for jacobian experiments.
Identical to eoss/utils/job_runner.py but always invokes jacobian/config.py.
"""
...
# Always run jacobian/config.py, regardless of CWD
_CONFIG_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'config.py')


def job_to_args(overrides, gpu):
    """Convert a dict of overrides to CLI args for jacobian/config.py."""
    args = [sys.executable, _CONFIG_SCRIPT, '--gpu', str(gpu)]
